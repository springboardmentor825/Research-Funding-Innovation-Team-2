...
const STARTUP_FUNDING = [
  { quarter: "Q1", seed: 2.5, seriesA: 0, seriesB: 0 },
  { quarter: "Q2", seed: 1.2, seriesA: 5.4, seriesB: 0 },
  { quarter: "Q3", seed: 0.8, seriesA: 8.2, seriesB: 12.5 },
  { quarter: "Q4", seed: 3.1, seriesA: 4.0, seriesB: 20.0 },
];

const PORTFOLIO_HEALTH = [
  { id: "Proj Alpha", risk: 20, roi: 85, budget: 1.2, fill: "#b5f333" },
  { id: "Proj Beta",  risk: 65, roi: 40, budget: 0.8, fill: "#fb923c" },
  { id: "Proj Gamma", risk: 80, roi: 95, budget: 2.5, fill: "#00d8ff" },
  { id: "Proj Delta", risk: 35, roi: 60, budget: 1.5, fill: "#a78bfa" },
];

const TEAM_METRICS = [
  { team: "Core Eng", velocity: 85, bugs: 12 },
  { team: "Data Sci", velocity: 92, bugs: 4 },
  { team: "UI/UX",    velocity: 78, bugs: 8 },
  { team: "Ops",      velocity: 88, bugs: 2 },
];

const PLATFORM_USAGE = [
  { day: "Mon", users: 1200, apiCalls: 8500 },
  { day: "Tue", users: 1350, apiCalls: 9200 },
  { day: "Wed", users: 1600, apiCalls: 11000 },
  { day: "Thu", users: 1420, apiCalls: 9800 },
  { day: "Fri", users: 1800, apiCalls: 13500 },
  { day: "Sat", users: 800,  apiCalls: 4000 },
  { day: "Sun", users: 950,  apiCalls: 4500 },
];

const USERS_LIST = [
  { name: "Alice Chen",   role: "Admin",              lastActive: "2 mins ago",  status: "Online" },
  { name: "Bob Smith",    role: "Innovation Manager", lastActive: "1 hour ago",  status: "Away" },
  { name: "Dr. J. Doe",   role: "Researcher",         lastActive: "5 hours ago", status: "Offline" },
  { name: "S. Williams",  role: "Startup",            lastActive: "Just now",    status: "Online" },
  { name: "K. Patel",     role: "Researcher",         lastActive: "2 days ago",  status: "Offline" },
];
...
function FundingView() {
  ...
  const toggleGrant = id => setAppliedGrants(current => current.includes(id) ? current.filter(grantId => grantId !== id) : [...current, id]);
  return h("div", null,
    h("div", { className:"stat-row" },
          ...
          h("tbody", null,
            GRANTS.map(g =>
              h("tr", { key: g.id },
                ...
                h("td", null, h("button", { className:"apply-btn", onClick:() => toggleGrant(g.id) }, appliedGrants.includes(g.id) ? "Applied" : "Apply"))
              )
            )
          )
        )
...
function PlaceholderView({ title }) {
  return h("div", { className:"panel", style:{ minHeight: 400, display:"flex", flexDirection:"column", justifyContent:"center", alignItems:"center" } },
    h("div", { className:"section-title", style:{ marginBottom: 16 } }, title),
    h("div", { style:{ color:"#7d8590", fontSize: 16 } }, 
      "This module is currently being configured for your persona."
    )
  );
}
...
function VentureCapitalFundingView() {
  return h("div", null,
    h("div", { className:"stat-row" },
      h(StatCard, { label:"Total Raised", value:"$32.5M", accent:"#b5f333" }),
      h(StatCard, { label:"Current Runway", value:"18 Months", accent:"#00d8ff" }),
      h(StatCard, { label:"Burn Rate", value:"$450K/mo", accent:"#fb923c" }),
    ),
    h("div", { className:"panel" },
      h("div", { className:"section-title" }, "Funding History by Quarter"),
      h(ResponsiveContainer, { width:"100%", height:300 },
        h(BarChart, { data: STARTUP_FUNDING },
          h(CartesianGrid, { strokeDasharray:"3 3", stroke:"#21262d", vertical:false }),
          h(XAxis, { dataKey:"quarter", tick:{ fill:"#7d8590", fontSize:11 }, axisLine:false, tickLine:false }),
          h(YAxis, { tick:{ fill:"#7d8590", fontSize:11 }, axisLine:false, tickLine:false }),
          h(Tooltip, { content: h(CustomTooltip) }),
          h(Bar, { dataKey:"seed", stackId:"a", fill:"#b5f333", name:"Seed Round" }),
          h(Bar, { dataKey:"seriesA", stackId:"a", fill:"#00d8ff", name:"Series A" }),
          h(Bar, { dataKey:"seriesB", stackId:"a", radius:[4,4,0,0], fill:"#a78bfa", name:"Series B" })
        )
      )
    )
  );
...
function TechStackAnalysisView() {
  return h("div", { className:"grid-2-col" },
    h("div", { className:"panel" },
      h("div", { className:"section-title" }, "Current Tech Stack"),
      h(ResponsiveContainer, { width:"100%", height:250 },
        h(PieChart, null,
          h(Pie, { data: licenseData, cx:"50%", cy:"50%", innerRadius:60, outerRadius:90, dataKey:"value", stroke:"none", paddingAngle:2 },
            licenseData.map((_, i) => h(Cell, { key:i, fill: PIE_COLORS[i] }))
          ),
          h(Tooltip, { content: h(CustomTooltip) })
        )
    ...
    h("div", { className:"panel" },
      h("div", { className:"section-title" }, "Core Technologies"),
      h("div", { className:"table-scroll" },
        h("table", { className:"data-table" },
          h("thead", null, h("tr", null, h("th", null, "Name"), h("th", null, "Category"), h("th", null, "Status"))),
          h("tbody", null,
            RAW.slice(0, 5).map((d, i) =>
              h("tr", { key:i },
                h("td", { style:{ color:"#e6edf3", fontWeight:600 } }, d.name),
                h("td", { style:{ color:"#7d8590" } }, d.cat),
                h("td", { style:{ color:"#b5f333" } }, "Active")
              )
            )
          )
        )
...
// 3. Commercialization Pipeline
function CommercializationPipelineView() {
  const [selectedStep, setSelectedStep] = useState("Prototyping");
  const steps = [
    { name:"Ideation", count:24, color:"#a78bfa", detail:"24 concepts are being evaluated against customer need and strategic fit." },
    { name:"Prototyping", count:12, color:"#00d8ff", detail:"12 concepts have working prototypes and are ready for user feedback." },
    { name:"Testing", count:5, color:"#fb923c", detail:"5 products are completing validation, security, and readiness checks." },
    { name:"Market Launch", count:2, color:"#b5f333", detail:"2 products are launch-ready with active commercialization plans." }
  ];
  const selected = steps.find(step => step.name === selectedStep);
  return h("div", { className:"panel" },
    h("div", { className:"section-title" }, "Commercialization Funnel"),
    h("div", { style:{ display:"flex", gap:16, marginTop:24, flexWrap:"wrap" } },
      steps.map(s => 
        h("button", { key:s.name, type:"button", onClick:() => setSelectedStep(s.name), style:{ flex:"1 1 200px", padding:24, background:selectedStep === s.name ? "#2a323c" : "#222831", border:"1px solid transparent", borderTop:`4px solid ${s.color}`, borderRadius:8, textAlign:"center", cursor:"pointer", color:"inherit" } },
          h("div", { style:{ fontSize:36, fontWeight:800, color:s.color, marginBottom:8, fontFamily:"Space Grotesk" } }, s.count),
          h("div", { style:{ color:"#e6edf3", fontWeight:600 } }, s.name)
        )
      )
    ),
    h("div", { style:{ marginTop:20, padding:16, background:"#1c2128", borderLeft:`3px solid ${selected.color}`, color:"#7d8590", lineHeight:1.6 } },
      h("strong", { style:{ color:selected.color } }, `${selected.name}: `), selected.detail
    )
  );
}

// 4. Portfolio Health
function PortfolioHealthView() {
  return h("div", null,
    h("div", { className:"stat-row" },
      h(StatCard, { label:"Total Projects", value:"18", accent:"#00d8ff" }),
      h(StatCard, { label:"At Risk", value:"2", accent:"#fb923c" }),
      h(StatCard, { label:"Avg ROI", value:"64%", accent:"#b5f333" }),
    ),
    h("div", { className:"panel" },
      h("div", { className:"section-title" }, "Risk vs ROI Analysis"),
      h(ResponsiveContainer, { width:"100%", height:300 },
        h(Recharts.ScatterChart, { margin:{ top:20, right:20, bottom:20, left:20 } },
          h(CartesianGrid, { strokeDasharray:"3 3", stroke:"#21262d" }),
          h(XAxis, { type:"number", dataKey:"risk", name:"Risk Score", tick:{ fill:"#7d8590" }, axisLine:false, tickLine:false }),
          h(YAxis, { type:"number", dataKey:"roi", name:"ROI %", tick:{ fill:"#7d8590" }, axisLine:false, tickLine:false }),
          h(Tooltip, { cursor:{ strokeDasharray:"3 3" }, contentStyle:{ background:"#161b22", border:"1px solid #30363d", borderRadius:4 } }),
          h(Recharts.Scatter, { name:"Projects", data: PORTFOLIO_HEALTH },
            PORTFOLIO_HEALTH.map((entry, index) => h(Cell, { key:index, fill: entry.fill }))
          )
        )
      )
    )
...
function InnovationPipelineView() {
  const [filter, setFilter] = useState("All");
  const filteredProjects = PORTFOLIO_HEALTH.filter(p => filter === "All" || (p.risk > 50 ? "At Risk" : "On Track") === filter);
  return h("div", { className:"panel" },
    h("div", { className:"section-title", style:{ display:"flex", justifyContent:"space-between", alignItems:"center" } },
      "Active R&D Projects",
      h("select", { className:"filter-select", value:filter, onChange:e => setFilter(e.target.value), style:{ padding:"6px 10px", width:"auto", textTransform:"none", letterSpacing:0 } },
        h("option", null, "All"), h("option", null, "On Track"), h("option", null, "At Risk")
      )
    ),
    h("div", { className:"table-scroll", style:{ marginTop:16 } },
      h("table", { className:"grants-table" },
        h("thead", null, h("tr", null, h("th", null,"Project"), h("th", null,"Phase"), h("th", null,"Budget"), h("th", null,"Status"))),
        h("tbody", null,
          filteredProjects.map(p =>
            h("tr", { key:p.id },
              h("td", { style:{ fontWeight:600, color:"#e6edf3" } }, p.id),
              h("td", { style:{ color:"#7d8590" } }, "Development"),
              h("td", { style:{ color:"#a78bfa" } }, `$${p.budget}M`),
              h("td", null, h("span", { className:"badge", style:{ background:`${p.fill}20`, color:p.fill } }, p.risk > 50 ? "At Risk" : "On Track"))
            )
          )
        )
      )
...
// 6. Team Metrics
function TeamMetricsView() {
  return h("div", { className:"panel" },
    h("div", { className:"section-title" }, "Team Velocity vs Bug Rate"),
    h(ResponsiveContainer, { width:"100%", height:300 },
      h(BarChart, { data: TEAM_METRICS },
        h(CartesianGrid, { strokeDasharray:"3 3", stroke:"#21262d", vertical:false }),
        h(XAxis, { dataKey:"team", tick:{ fill:"#7d8590", fontSize:11 }, axisLine:false, tickLine:false }),
        h(YAxis, { tick:{ fill:"#7d8590", fontSize:11 }, axisLine:false, tickLine:false }),
        h(Tooltip, { content: h(CustomTooltip) }),
        h(Bar, { dataKey:"velocity", name:"Velocity", fill:"#00d8ff", radius:[4,4,0,0] }),
        h(Bar, { dataKey:"bugs", name:"Bugs", fill:"#fb923c", radius:[4,4,0,0] })
      )
    )
  );
}
...
function PlatformUsageView() {
  return h("div", { className:"panel" },
    h("div", { className:"section-title" }, "Weekly Active Users & API Calls"),
    h(ResponsiveContainer, { width:"100%", height:300 },
      h(LineChart, { data: PLATFORM_USAGE },
        h(CartesianGrid, { strokeDasharray:"3 3", stroke:"#21262d", vertical:false }),
        h(XAxis, { dataKey:"day", tick:{ fill:"#7d8590", fontSize:11 }, axisLine:false, tickLine:false }),
        h(YAxis, { yAxisId:"left", tick:{ fill:"#7d8590", fontSize:11 }, axisLine:false, tickLine:false }),
        h(YAxis, { yAxisId:"right", orientation:"right", tick:{ fill:"#7d8590", fontSize:11 }, axisLine:false, tickLine:false }),
        h(Tooltip, { content: h(CustomTooltip) }),
        h(Line, { yAxisId:"left", type:"monotone", dataKey:"users", name:"Users", stroke:"#b5f333", strokeWidth:3 }),
        h(Line, { yAxisId:"right", type:"monotone", dataKey:"apiCalls", name:"API Calls", stroke:"#a78bfa", strokeWidth:3 })
      )
    )
  );
}
...
function UserGovernanceView() {
  ...
  const addUser = () => {
    ...
    setUsers(current => [...current, { name:name.trim(), role:"Researcher", lastActive:"Just now", status:"Online" }]);
  };
  const toggleStatus = name => setUsers(current => current.map(user => user.name === name ? { ...user, status:user.status === "Online" ? "Offline" : "Online", lastActive:"Just now" } : user));
  return h("div", { className:"panel" },
    h("div", { className:"section-title", style:{ display:"flex", justifyContent:"space-between", alignItems:"center" } }, 
      "User Management",
      h("div", { style:{ display:"flex", gap:8, alignItems:"center" } },
        h("select", { className:"filter-select", value:statusFilter, onChange:e => setStatusFilter(e.target.value), style:{ padding:"6px 10px", width:"auto", textTransform:"none", letterSpacing:0 } },
          h("option", null, "All"), h("option", null, "Online"), h("option", null, "Away"), h("option", null, "Offline")
        ),
        h("button", { className:"apply-btn", onClick:addUser, style:{ padding:"6px 14px", background:"#00d8ff", color:"#000", border:"none", borderRadius:6, cursor:"pointer", fontWeight:600 } }, "+ Add User")
      )
    ),
    h("div", { className:"table-scroll", style:{ marginTop:16 } },
      h("table", { className:"data-table" },
        h("thead", null, h("tr", null, h("th", null,"Name"), h("th", null,"Role"), h("th", null,"Last Active"), h("th", null,"Status"), h("th", null,"Actions"))),
        h("tbody", null,
          visibleUsers.map((u, i) =>
            h("tr", { key:i },
              h("td", { style:{ fontWeight:600, color:"#e6edf3" } }, u.name),
              h("td", { style:{ color:"#7d8590" } }, u.role),
              h("td", { style:{ color:"#a78bfa", fontSize:12 } }, u.lastActive),
              h("td", null, h("span", { className:"badge", style:{ background:u.status==="Online"?"#b5f33320":"#21262d", color:u.status==="Online"?"#b5f333":"#7d8590" } }, u.status)),
              h("td", null, h("button", { onClick:() => toggleStatus(u.name), style:{ background:"transparent", border:"1px solid #30363d", color:"#e6edf3", borderRadius:4, padding:"4px 10px", cursor:"pointer" } }, u.status === "Online" ? "Disable" : "Enable"))
            )
          )
        )
      )
...
function SystemSettingsView() {
  ...
  const saveConfiguration = () => setSaved(true);
  return h("div", { className:"panel", style:{ maxWidth:600 } },
    h("div", { className:"section-title" }, "Global Configuration"),
    h("div", { style:{ display:"flex", flexDirection:"column", gap:20, marginTop:24 } },
      h("div", null,
        h("label", { style:{ color:"#e6edf3", display:"block", marginBottom:8, fontWeight:600, fontSize:14 } }, "API Rate Limit (req/min)"),
        h("input", { type:"number", value:rateLimit, onChange:e => { setRateLimit(e.target.value); setSaved(false); }, className:"search-input", style:{ width:"100%" } })
      ),
      h("div", null,
        h("label", { style:{ color:"#e6edf3", display:"block", marginBottom:8, fontWeight:600, fontSize:14 } }, "Data Retention Period (Days)"),
        h("select", { className:"filter-select", value:retention, onChange:e => { setRetention(e.target.value); setSaved(false); }, style:{ width:"100%", padding:"10px 12px" } },
          h("option", null, "30 Days"), h("option", null, "90 Days"), h("option", null, "1 Year")
        )
      ),
      h("div", { style:{ display:"flex", alignItems:"center", gap:12, marginTop:8 } },
        h("input", { type:"checkbox", checked:twoFactor, onChange:e => { setTwoFactor(e.target.checked); setSaved(false); }, style:{ width:18, height:18, accentColor:"#b5f333", cursor:"pointer" } }),
        h("span", { style:{ color:"#7d8590", fontSize:14 } }, "Enable Two-Factor Authentication for all Admins")
      ),
      h("div", { style:{ marginTop:16, borderTop:"1px solid #21262d", paddingTop:20 } },
        h("button", { className:"apply-btn", onClick:saveConfiguration, style:{ padding:"10px 20px" } }, "Save Configuration"),
        saved && h("span", { style:{ marginLeft:12, color:"#b5f333", fontSize:12 } }, "Configuration saved")
      )
    )
  );
}
...
function Dashboard() {
  ...
  const [userName, setUserName]           = useState("M Nishandhi");
  const [userInitials, setUserInitials]   = useState("MN");

  useEffect(() => {
    // Retrieve username from localStorage or sessionStorage after login
    const storedName = localStorage.getItem("username") || localStorage.getItem("user") || localStorage.getItem("name") || 
                       sessionStorage.getItem("username") || sessionStorage.getItem("user") || sessionStorage.getItem("name");
    
    if (storedName) {
      setUserName(storedName);
      const initials = storedName.split(' ').map(n => n[0]).filter(c => /[a-zA-Z]/.test(c)).join('').toUpperCase().substring(0, 2);
  ...
  const tabSections = {
    researcher: ["Overview", "Funding Recommendations", "Research Trends", "Publication Analytics", "Patent Insights", "Innovation Score"],
    startup: ["Overview", "Venture Capital Funding", "Tech Stack Analysis", "Commercialization Pipeline"],
    manager: ["Overview", "Portfolio Health", "Innovation Pipeline", "Team Metrics"],
    admin: ["Overview", "Platform Usage", "User Governance", "System Settings"]
  };

  const sections = tabSections[activeTab] || tabSections.researcher;
  ...
  return h("div", { className:"layout" },
    ...
    h("nav", { className:"sidebar" },
      ...
      h("div", { className:"sidebar-user" },
        h("div", { className:"user-avatar" }, userInitials),
        h("div", null,
          h("div", { className:"user-name" }, userName),
          h("div", { className:"user-role" }, activeTabObj?.label || "Researcher")
        ),
    ...
    h("main", { className:"main" },
      ...
      activeSection === "Venture Capital Funding" ? h(VentureCapitalFundingView) :
      activeSection === "Tech Stack Analysis"     ? h(TechStackAnalysisView) :
      activeSection === "Commercialization Pipeline"? h(CommercializationPipelineView) :
      activeSection === "Portfolio Health"        ? h(PortfolioHealthView) :
      activeSection === "Innovation Pipeline"     ? h(InnovationPipelineView) :
      activeSection === "Team Metrics"            ? h(TeamMetricsView) :
      activeSection === "Platform Usage"          ? h(PlatformUsageView) :
      activeSection === "User Governance"         ? h(UserGovernanceView) :
      activeSection === "System Settings"         ? h(SystemSettingsView) :
      h(PlaceholderView, { title: activeSection }),

      // Footer
      h("div", { className:"footer" },
        `Innovation Intelligence Platform · ${userName} · Dataset: ${totalTech} technologies · ${new Date().getFullYear()}`
      )
    )
  );
}
